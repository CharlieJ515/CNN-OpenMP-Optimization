...
public:
	...
	void test(string filename_input, string filename_output)
	{
		...
		// (4) 할당 해제
		free(pLoadImage);
		// input_img_Y is deallocated with first tensor
		// delete[] input_img_Y;
		delete[] input_img_U;
		delete[] input_img_V;
	}
	...
	void read_image(const string filename, BYTE *&pLoadImage, double *&img_Y, double *&img_U, double *&img_V, int &nH, int &nW)
	{
		...
		img_Y = new double[nH* nW];
		img_U = new double[nH* nW];
		img_V = new double[nH* nW];

		convertRGBtoYUV(pLoadImage, img_Y, img_U, img_V, nH, nW);

		Tensor3D *temp = new Tensor3D(nH, nW, 1);
		temp->set_tensor(img_Y);
	...
	{
		double *img_Y = tensor_Y->get_tensor();
		convertYUVtoRGB(img_Y, img_U, img_V, pLoadImage, nH, nW);
		SaveBmp(filename.c_str(), pLoadImage, nH, nW);
	}
	void print_layer_info() const
	{
		cout << endl
			 << "(Layer information)_____________" << endl;
	...
	{
		cout << endl
			 << "(Tensor information)_____________" << endl;
