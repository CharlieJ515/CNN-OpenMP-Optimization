...
bool convertRGBtoYUV(BYTE *src, double *dst_Y, double *dst_U, double *dst_V, int height, int width)
{
	for (int i = 0; i < height * width; i++)
	{
		int idx_src = 3 * i;

		int iB = src[idx_src + 0];
		int iG = src[idx_src + 1];
		int iR = src[idx_src + 2];

		double Y = iR * 0.299 + iG * 0.587 + iB * 0.114;

		dst_Y[i] = Y / 255.0; // normalize
		dst_U[i] = (iB - Y) * 0.565;
		dst_V[i] = (iR - Y) * 0.713;
	}
	return true;
...
bool convertYUVtoRGB(double *src_Y, double *src_U, double *src_V, BYTE *dst, int height, int width)
{
	for (int i = 0; i < width * height; i++)
	{
		// Undo normalization: [0–1] → [0–255]
		double Y = src_Y[i] * 255.0;

		double R = clip(Y + 1.403 * src_V[i], 0.0, 255.0);
		double G = clip(Y - 0.344 * src_U[i] - 0.714 * src_V[i], 0.0, 255.0);
		double B = clip(Y + 1.770 * src_U[i], 0.0, 255.0);

		int rgbIndex = 3 * i;
		dst[rgbIndex + 0] = (BYTE)B;
		dst[rgbIndex + 1] = (BYTE)G;
		dst[rgbIndex + 2] = (BYTE)R;
	}
